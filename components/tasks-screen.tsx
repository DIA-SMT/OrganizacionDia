    ...
    <AppShell title="Tareas" subtitle="Trabajo tecnico asociado a los proyectos" search={search} onSearchChange={setSearch}>
      {loading ? (
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="hidden grid-cols-[1.2fr_0.8fr_0.6fr_0.6fr_0.8fr] gap-3 border-b border-slate-100 px-4 py-3 text-xs font-semibold uppercase text-slate-400 dark:border-slate-800 dark:text-slate-500 lg:grid">
            <span>Tarea</span>
            <span>Proyecto</span>
            <span>Tipo</span>
            <span>Estado</span>
          ...
          {filtered.map((task) => (
            <div key={task.id} className="grid gap-3 border-b border-slate-100 px-4 py-4 text-sm last:border-b-0 dark:border-slate-800 lg:grid-cols-[1.2fr_0.8fr_0.6fr_0.6fr_0.8fr]">
              <div>
